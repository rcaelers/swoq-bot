...
use crate::planners::heuristic::strategies::*;
...
impl StrategyPlanner {
    ...
    pub fn select_goal(&mut self, state: &PlannerState) -> Vec<Goal> {
        let num_players = state.world.players.len();
        ...
        match strategy.strategy_type() {
            StrategyType::Individual => {
                for (player_index, goal_slot) in selected_goals.iter_mut().enumerate() {
                    if goal_slot.is_none()
                        && let Some(goal) = strategy.try_select(state, player_index)
                    {
                        if is_prioritized {
                            debug!(
            ...
            StrategyType::Coop => {
                if Self::any_player_needs_goal(selected_goals) {
                    let coop_goals = strategy.try_select_coop(state, selected_goals);
