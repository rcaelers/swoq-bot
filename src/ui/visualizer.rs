...
fn render_world_state(
    ...
    for (pos, tile) in world_state.map.iter() {
        ...
        let color = match tile {
            Tile::DoorRed if world_state.is_door_open(crate::infra::Color::Red) => {
                Color::srgba(1.0, 1.0, 1.0, 0.4) // 40% opacity for open doors
            }
            Tile::DoorGreen if world_state.is_door_open(crate::infra::Color::Green) => {
                Color::srgba(1.0, 1.0, 1.0, 0.4)
            }
            Tile::DoorBlue if world_state.is_door_open(crate::infra::Color::Blue) => {
                Color::srgba(1.0, 1.0, 1.0, 0.4)
            }
    ...
    for (color, boulder_positions) in boulders_on_plates.iter() {
        let border_color = match color {
            crate::infra::Color::Red => Color::srgb(1.0, 0.0, 0.0),
            crate::infra::Color::Green => Color::srgb(0.0, 1.0, 0.0),
            crate::infra::Color::Blue => Color::srgb(0.0, 0.5, 1.0),
        };
    ...
    if world_state.players.len() > 1 {
        ...
        let p2_goal = snapshot.p2_goal.as_deref().unwrap_or("N/A");
...
fn update_log_pane(
    ...
    // Only update if we have new messages OR the log pane doesn't exist
    let log_pane_exists = !log_pane_query.is_empty();
    if !has_new_messages && log_pane_exists {
        return;
    }
